"""
Scraper para El Colombiano - Medellín / Antioquia
...
class ElColombianoScraper(BaseScraper):
    """Scraper para El Colombiano - principal de Medellín"""

    def __init__(self):
        super().__init__(
            source_name="elcolombiano",
    ...
    async def scrape_frontpage(self) -> List[ScrapedArticle]:
        ...
        logger.info(f"El Colombiano: {len(unique)} artículos únicos encontrados")
    ...
    async def _scrape_section(self, url: str) -> List[ScrapedArticle]:
        ...
        for tag in soup.find_all('div', class_=re.compile(r'(article|card|nota|story|priority)', re.I), limit=30):
            article = self._extract(tag)
            if article:
                articles.append(article)

        for link in soup.find_all('a', href=re.compile(r'elcolombiano\.com/.+\d+'), limit=40):
    ...
    def _extract(self, tag) -> Optional[ScrapedArticle]:
        try:
            ...
            logger.debug(f"El Colombiano error: {e}")
            return None
