"""
Scraper para Pulzo - Medio digital colombiano
Fuente: pulzo.com
"""
...
class PulzoScraper(BaseScraper):
    """Scraper para Pulzo - digital nativo colombiano"""

    def __init__(self):
        super().__init__(
            source_name="pulzo",
            base_url="https://www.pulzo.com",
            language='es',
    ...
    async def scrape_frontpage(self) -> List[ScrapedArticle]:
        ...
        logger.info(f"Pulzo: {len(unique)} artículos únicos encontrados")
    ...
    async def _scrape_section(self, url: str) -> List[ScrapedArticle]:
        ...
        for div in soup.find_all('div', class_=re.compile(r'(card|article|nota|noticias)', re.I), limit=30):
            article = self._extract_from_tag(div)
            if article:
                articles.append(article)

        # Pulzo uses direct links with specific patterns
        for link in soup.find_all('a', href=re.compile(r'pulzo\.com/.+/[A-Z]{2}\d+'), limit=40):
    ...
    def _extract_from_tag(self, tag) -> Optional[ScrapedArticle]:
        try:
            ...
            logger.debug(f"Pulzo extract error: {e}")
            return None
    ...
    def _extract_article(self, link) -> Optional[ScrapedArticle]:
        try:
            ...
            logger.debug(f"Pulzo article error: {e}")
            return None
