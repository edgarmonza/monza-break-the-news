"""
Scraper para Vanguardia - Bucaramanga / Santander
...
class VanguardiaScraper(BaseScraper):
    """Scraper para Vanguardia - principal de Santander"""

    def __init__(self):
        super().__init__(
            source_name="vanguardia",
    ...
    async def scrape_frontpage(self) -> List[ScrapedArticle]:
        ...
        logger.info(f"Vanguardia: {len(unique)} artículos únicos encontrados")
    ...
    async def _scrape_section(self, url: str) -> List[ScrapedArticle]:
        ...
        for link in soup.find_all('a', href=re.compile(r'vanguardia\.com/.+'), limit=40):
    ...
    def _extract(self, tag) -> Optional[ScrapedArticle]:
        try:
            ...
            logger.debug(f"Vanguardia error: {e}")
            return None
