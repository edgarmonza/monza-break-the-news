"""
Scraper para El Heraldo - Barranquilla / Costa Caribe
Fuente: elheraldo.co
"""
...
class ElHeraldoScraper(BaseScraper):
    """Scraper para El Heraldo - principal de la Costa Caribe"""

    def __init__(self):
        super().__init__(
            source_name="elheraldo",
    ...
    async def scrape_frontpage(self) -> List[ScrapedArticle]:
        ...
        logger.info(f"El Heraldo: {len(unique)} artículos únicos encontrados")
    ...
    async def _scrape_section(self, url: str) -> List[ScrapedArticle]:
        ...
        for link in soup.find_all('a', href=re.compile(r'elheraldo\.co/.+\d+'), limit=40):
    ...
    def _extract(self, tag) -> Optional[ScrapedArticle]:
        try:
            ...
            logger.debug(f"El Heraldo error: {e}")
            return None
