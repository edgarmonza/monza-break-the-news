"""
Scraper para La Opinión - Cúcuta / Norte de Santander
...
class LaOpinionScraper(BaseScraper):
    """Scraper para La Opinión - principal de Cúcuta / frontera"""

    def __init__(self):
        super().__init__(
            source_name="laopinion",
            base_url="https://www.laopinion.com.co",
            language='es',
    ...
    async def scrape_frontpage(self) -> List[ScrapedArticle]:
        ...
        logger.info(f"La Opinión: {len(unique)} artículos únicos encontrados")
    ...
    async def _scrape_section(self, url: str) -> List[ScrapedArticle]:
        ...
        for link in soup.find_all('a', href=re.compile(r'laopinion\.com\.co/.+'), limit=40):
    ...
    def _extract(self, tag) -> Optional[ScrapedArticle]:
        try:
            ...
            logger.debug(f"La Opinión error: {e}")
            return None
